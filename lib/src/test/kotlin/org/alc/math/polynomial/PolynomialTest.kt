...
class PolynomialTest {
    @Test
    fun apply() {
        val p = Polynomial.create(0, 2, 1)
        assertEquals(3.0, p.apply(1.0))
        assertIs<Polynomial.Linear>(p)

        val q = Polynomial.create(1, -1, -2)
        assertEquals(4.0, q.apply(3.0))
        assertIs<Polynomial.Quadratic>(q)
    }
    ...
    fun degree() {
        ...
        val q = Polynomial.create(0, 1, -1, -2)
    ...
    @Test
    fun derivative() {
        val p = Polynomial.create(3, 2, 5, 5)
        assertEquals(3, p.degree())
        val q = p.derivative()
        assertEquals(2, q.degree())
        assertIs<Polynomial.Quadratic>(q)
        val q1 = Polynomial.create(9, 4, 5)
        assertEquals(q1, q)
    }

    @Test
    fun root2() {
        val p = Polynomial.create(1, 0, -2)
    ...
    @Test
    fun rootDegree1() {
        val p = Polynomial.create(1, -2)
        assertEquals(2.0, p.root())
        val p1 = Polynomial.create(2, 1)
        assertEquals(-0.5, p1.root())
    }

    @Test
    fun golden() {
        val p = Polynomial.create(1, -1, -1)
    ...
    @Test
    fun fourthRootOf25() {
        val p = Polynomial.create(1, 0, 0, 0, -25)
        assertEquals(sqrt(5.0), p.root())
    }

    @Test
    fun toStringTest() {
        val p = Polynomial.create(1.0, -1.0, 3.5, -2.0)
    ...
    @Test
    fun zero() {
        val p = Polynomial.create(0)
        assertSame(Polynomial.ZERO, p)
        assertIs<Polynomial.Constant>(p)
    }

    @Test
    fun add() {
        val p1 = Polynomial.create(1, 0, -2)
        val p2 = Polynomial.create(-1, 0, 2)
        val p3 = Polynomial.create(-2, 0, 4)
    ...
    fun unaryMinus() {
        val p1 = Polynomial.create(1, 0, -2)
        assertEquals(Polynomial.create(-1, 0, 2), -p1)
        assertSame(Polynomial.ZERO, p1 + (-p1))
        assertSame(Polynomial.ZERO, -Polynomial.ZERO)
    }
    ...
    fun negate() {
        val p1 = Polynomial.create(1, 0, -2)
        assertEquals(Polynomial.create(-1, 0, 2), p1.negate())
    ...
    fun multiplyZeroDegree() {
        val p1 = Polynomial.create(1, 0, -2)
        val p2 = Polynomial.create(2)
        assertEquals(Polynomial.create(2, 0, -4), p1 * p2)
        assertEquals(Polynomial.create(2, 0, -4), p2 * p1)

        val p3 = Polynomial.create(-3)
        assertEquals(Polynomial.create(-6), p2 * p3)
        assertEquals(Polynomial.create(-6), p3 * p2)
    }
    ...
    fun multiply() {
        ...
        // (x+1) * (x - 1) = x^2 - 1
        val p1 = Polynomial.create(1, 1)
        val q = Polynomial.create(1, -1)
        val pq = Polynomial.create(1, 0, -1)
        assertEquals(pq, p1 * q)
        assertEquals(pq, q * p1)

        // (x+1)^2 = x^2 + 2x + 1
        val p2 = Polynomial.create(1, 2, 1)
        assertEquals(p2, p1 * p1)

        // (x + 1)^3 = x^3 + 3x^2 + 3x + 1
        val p3 = Polynomial.create(1, 3, 3, 1)
        assertEquals(p3, p1 * p2)
        assertEquals(p3, p2 * p1)

        // (x + 1)^4 = x^4 + 4x^3 + 6x^2 + 4x + 1
        val p4 = Polynomial.create(1, 4, 6, 4, 1)
    ...
    fun divideAndRemainder1() {
        val num = Polynomial.create(2, 1)
        val den = Polynomial.create(1, 2)
        val pair = num.divideAndRemainder(den)
        assertEquals(Polynomial.create(2), pair.first)
        assertEquals(Polynomial.create(-3), pair.second)
    }

    @Test
    fun divideAndRemainder2() {
        val num = Polynomial.create(1, 0, -1)
        val den = Polynomial.create(1, -1)
        val pair = num.divideAndRemainder(den)
        assertEquals(Polynomial.create(1, 1), pair.first)
        assertEquals(Polynomial.ZERO, pair.second)
    }

    @Test
    fun divideAndRemainder3() {
        val num = Polynomial.CUBE * Polynomial.create(0.5)
        val den = Polynomial.CUBE
        val pair = num.divideAndRemainder(den)
        assertEquals(Polynomial.create(0.5), pair.first)
        assertEquals(Polynomial.ZERO, pair.second)
    }

    @Test
    fun minus0() {
        val p1 = Polynomial.create(-0.0)
        assertSame(Polynomial.ZERO, p1)
        val p2 = Polynomial.create(1.0, -0.0)
        assertSame(Polynomial.IDENTITY, p2)
        val p3 = Polynomial.create(1.0, -0.0, -0.0)
    ...
    fun linearInterpolation() {
        ...
        val expected = Polynomial.create(2.0, 3.0)
    ...
    fun polynomialInterpolation() {
        ...
        val expected = Polynomial.create(1, -1, -1)
        assertEquals(expected, p)
