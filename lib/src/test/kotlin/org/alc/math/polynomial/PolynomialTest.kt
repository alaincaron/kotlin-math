...
class PolynomialTest {
    @Test
    fun apply() {
        val p = Polynomial.fromInts(0, 2, 1)
        assertEquals(3.0, p.apply(1.0))

        val q = Polynomial.fromInts(1, -1, -1)
    ...
    fun degree() {
        ...
        val q = Polynomial.fromInts(0, 1, -1, -1)
        assertEquals(2, q.degree())
    }

    @Test
    fun derivative() {
        val p = Polynomial.fromInts(3, 2, 5, 5)
        assertEquals(3, p.degree())
        val q = p.derivative()
        assertEquals(2, q.degree())
        val q1 = Polynomial.fromInts(9,4,5)
        assertEquals(q1, q)
    }

    @Test
    fun root2() {
        val p = Polynomial.fromInts(1, 0, -2)
    ...
    @Test
    fun golden() {
        val p = Polynomial.fromInts(1, -1, -1)
    ...
    @Test
    fun toStringTest() {
        val p = Polynomial.fromDoubles(1.0, -1.0, 3.5, -2.0)
    ...
    @Test
    fun zero() {
        val p = Polynomial.fromInts(0)
        assertSame(Polynomial.ZERO, p)
    }

    @Test
    fun add() {
        val p1 = Polynomial.fromInts(1, 0, -2)
        val p2 = Polynomial.fromInts(-1, 0, 2)
        val p3 = Polynomial.fromInts(-2, 0, 4)
    ...
    fun unaryMinus() {
        val p1 = Polynomial.fromInts(1, 0, -2)
        assertEquals(Polynomial.fromInts(-1,0,2), -p1)
        assertSame(Polynomial.ZERO, p1 + (-p1))
        assertSame(Polynomial.ZERO, -Polynomial.ZERO)
    }
    ...
    fun negate() {
        val p1 = Polynomial.fromInts(1, 0, -2)
        assertEquals(Polynomial.fromInts(-1,0,2), p1.negate())
    ...
    fun multiplyZeroDegree() {
        val p1 = Polynomial.fromInts(1, 0, -2)
        val p2 = Polynomial.fromInts(2)
        assertEquals(Polynomial.fromInts(2,0,-4), p1 * p2)
        assertEquals(Polynomial.fromInts(2,0,-4), p2 * p1)

        val p3 = Polynomial.fromInts(-3)
        assertEquals(Polynomial.fromInts(-6), p2 * p3)
        assertEquals(Polynomial.fromInts(-6), p3 * p2)
    }
    ...
    fun multiply() {
        ...
        // (x+1) * (x - 1) = x^2 - 1
        val p1 = Polynomial.fromInts(1,1)
        val q = Polynomial.fromInts(1,-1)
        val pq = Polynomial.fromInts(1,0,-1)
        assertEquals(pq, p1 * q)
        assertEquals(pq, q * p1)

        // (x+1)^2 = x^2 + 2x + 1
        val p2 = Polynomial.fromInts(1,2,1)
        assertEquals(p2, p1 * p1)

        // (x + 1)^3 = x^3 + 3x^2 + 3x + 1
        val p3 = Polynomial.fromInts(1,3,3,1)
        assertEquals(p3, p1 * p2)
        assertEquals(p3, p2 * p1)

        // (x + 1)^4 = x^4 + 4x^3 + 6x^2 + 4x + 1
        val p4 = Polynomial.fromInts(1,4,6,4,1)
    ...
    fun divideAndRemainder1() {
        val num = Polynomial.fromInts(2,1)
        val den = Polynomial.fromInts(1, 2)
        val pair = num.divideAndRemainder(den)
        assertEquals(Polynomial.fromInts(2), pair.first)
        assertEquals(Polynomial.fromInts(-3), pair.second)
    }

    @Test
    fun divideAndRemainder2() {
        val num = Polynomial.fromInts(1,0,-1)
        val den = Polynomial.fromInts(1,-1)
        val pair = num.divideAndRemainder(den)
        assertEquals(Polynomial.fromInts(1,1), pair.first)
        assertEquals(Polynomial.ZERO, pair.second)
    }
    @Test
    fun divideAndRemainder3() {
        val num = Polynomial.CUBE * 0.5
        val den = Polynomial.CUBE
        val pair = num.divideAndRemainder(den)
        assertEquals(Polynomial.fromDoubles(0.5), pair.first)
        assertEquals(Polynomial.ZERO, pair.second)
    }

    @Test
    fun minus0() {
        val p1 = Polynomial.fromDoubles(-0.0)
        assertSame(Polynomial.ZERO, p1)
        val p2 = Polynomial.fromDoubles(1.0, -0.0)
        assertSame(Polynomial.IDENTITY, p2)
        val p3 = Polynomial.fromDoubles(1.0, -0.0, -0.0)
