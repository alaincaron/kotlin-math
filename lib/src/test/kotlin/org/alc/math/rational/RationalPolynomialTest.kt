...
class RationalPolynomialTest {
    @Test
    fun apply() {
        val p = RationalPolynomial.withCoefficients(0, 2, 1)
        assertEquals(3 over 1, p.apply(Rational.ONE))
        assertIs<LinearRationalPolynomial>(p)

        val q = RationalPolynomial.withCoefficients(1, -1, -2)
        assertEquals(4 over 1, q.apply(3 over 1))
        assertIs<QuadraticRationalPolynomial>(q)
    }
    ...
    fun degree() {
        ...
        val q = RationalPolynomial.withCoefficients(0, 1, -1, -2)
    ...
    @Test
    fun derivative() {
        val p = RationalPolynomial.withCoefficients(3, 2, 5, 5)
        assertEquals(3, p.degree())
        val q = p.derivative()
        assertEquals(2, q.degree())
        assertIs<QuadraticRationalPolynomial>(q)
        val q1 = RationalPolynomial.withCoefficients(9, 4, 5)
        assertEquals(q1, q)
    }

    @Test
    fun root2() {
        val p = RationalPolynomial.withCoefficients(1, 0, -2)
    ...
    @Test
    fun rootDegree1() {
        val p = RationalPolynomial.withCoefficients(1, -2)
        assertEquals(2.0, p.root())
        val p1 = RationalPolynomial.withCoefficients(2, 1)
        assertEquals(-0.5, p1.root())
    }

    @Test
    fun golden() {
        val p = RationalPolynomial.withCoefficients(1, -1, -1)
    ...
    @Test
    fun fourthRootOf25() {
        val p = RationalPolynomial.withCoefficients(1, 0, 0, 0, -25)
        assertEquals(sqrt(5.0), p.root())
    }

    @Test
    fun toStringTest() {
        val p = RationalPolynomial.withCoefficients(1.0, -1.0, 3.5, -2.0)
    ...
    @Test
    fun zero() {
        val p = RationalPolynomial.withCoefficients(0)
        assertSame(RationalPolynomial.ZERO, p)
        assertIs<ConstantRationalPolynomial>(p)
    }

    @Test
    fun add() {
        val p1 = RationalPolynomial.withCoefficients(1, 0, -2)
        val p2 = RationalPolynomial.withCoefficients(-1, 0, 2)
        val p3 = RationalPolynomial.withCoefficients(-2, 0, 4)
    ...
    fun unaryMinus() {
        val p1 = RationalPolynomial.withCoefficients(1, 0, -2)
        assertEquals(RationalPolynomial.withCoefficients(-1, 0, 2), -p1)
        assertSame(RationalPolynomial.ZERO, p1 + (-p1))
        assertSame(RationalPolynomial.ZERO, -RationalPolynomial.ZERO)
    }
    ...
    fun negate() {
        val p1 = RationalPolynomial.withCoefficients(1, 0, -2)
        assertEquals(RationalPolynomial.withCoefficients(-1, 0, 2), p1.negate())
    ...
    fun multiplyZeroDegree() {
        val p1 = RationalPolynomial.withCoefficients(1, 0, -2)
        val p2 = RationalPolynomial.withCoefficients(2)
        assertEquals(RationalPolynomial.withCoefficients(2, 0, -4), p1 * p2)
        assertEquals(RationalPolynomial.withCoefficients(2, 0, -4), p2 * p1)

        val p3 = RationalPolynomial.withCoefficients(-3)
        assertEquals(RationalPolynomial.withCoefficients(-6), p2 * p3)
        assertEquals(RationalPolynomial.withCoefficients(-6), p3 * p2)
    }
    ...
    fun multiply() {
        ...
        // (x+1) * (x - 1) = x^2 - 1
        val p1 = RationalPolynomial.withCoefficients(1, 1)
        val q = RationalPolynomial.withCoefficients(1, -1)
        val pq = RationalPolynomial.withCoefficients(1, 0, -1)
        assertEquals(pq, p1 * q)
        assertEquals(pq, q * p1)

        // (x+1)^2 = x^2 + 2x + 1
        val p2 = RationalPolynomial.withCoefficients(1, 2, 1)
        assertEquals(p2, p1 * p1)

        // (x + 1)^3 = x^3 + 3x^2 + 3x + 1
        val p3 = RationalPolynomial.withCoefficients(1, 3, 3, 1)
        assertEquals(p3, p1 * p2)
        assertEquals(p3, p2 * p1)

        // (x + 1)^4 = x^4 + 4x^3 + 6x^2 + 4x + 1
        val p4 = RationalPolynomial.withCoefficients(1, 4, 6, 4, 1)
    ...
    fun divideAndRemainder1() {
        val num = RationalPolynomial.withCoefficients(2, 1)
        val den = RationalPolynomial.withCoefficients(1, 2)
        val pair = num.divideAndRemainder(den)
        assertEquals(RationalPolynomial.withCoefficients(2), pair.first)
        assertEquals(RationalPolynomial.withCoefficients(-3), pair.second)
    }

    @Test
    fun divideAndRemainder2() {
        val num = RationalPolynomial.withCoefficients(1, 0, -1)
        val den = RationalPolynomial.withCoefficients(1, -1)
        val pair = num.divideAndRemainder(den)
        assertEquals(RationalPolynomial.withCoefficients(1, 1), pair.first)
        assertEquals(RationalPolynomial.ZERO, pair.second)
    }

    @Test
    fun divideAndRemainder3() {
        val num = RationalPolynomial.CUBE * RationalPolynomial.withCoefficients(0.5)
        val den = RationalPolynomial.CUBE
        val pair = num.divideAndRemainder(den)
        assertEquals(RationalPolynomial.withCoefficients(0.5), pair.first)
        assertEquals(RationalPolynomial.ZERO, pair.second)
    }

    @Test
    fun minus0() {
        val p1 = RationalPolynomial.withCoefficients(-0.0)
        assertSame(RationalPolynomial.ZERO, p1)
        val p2 = RationalPolynomial.withCoefficients(1.0, -0.0)
        assertSame(RationalPolynomial.IDENTITY, p2)
        val p3 = RationalPolynomial.withCoefficients(1.0, -0.0, -0.0)
    ...
    fun linearInterpolation() {
        ...
        val expected = RationalPolynomial.withCoefficients(2 over 1, 3 over 1)
    ...
    fun polynomialInterpolation() {
        ...
        val expected = RationalPolynomial.withCoefficients(1, -1, -1)
        assertEquals(expected, p)
